...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try { await logAudit({ user_id: body.actorId || 'u-admin', entity: 'config', entity_id: 'monitoring', action: 'config.monitoring.update', before_data: before, after_data: updated.monitoring, ip: getRequestIp(request), user_agent: request.headers.get('user-agent') || null }); } catch (e) {}
    return json(updated);
  } catch (e) {
