...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try { await logAudit({ user_id: body.actorId || 'u-admin', entity: 'config', entity_id: 'security', action: 'config.security.update', before_data: before, after_data: updated.security, ip: getRequestIp(request), user_agent: request.headers.get('user-agent') ?? undefined }); } catch (e) {}
    return json(updated);
  } catch (e) {
