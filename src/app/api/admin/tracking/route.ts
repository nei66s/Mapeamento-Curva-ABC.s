...
export async function POST(request: NextRequest) {
  ...
  try {
    await logAudit({ user_id: body.userId || 'u-admin', entity: 'action', entity_id: body.name || 'event', action: body.name || 'custom.event', before_data: null, after_data: body.payload || {}, ip: getRequestIp(request), user_agent: request.headers.get('user-agent') || null });
  } catch (e) {
    // ignore
  }
