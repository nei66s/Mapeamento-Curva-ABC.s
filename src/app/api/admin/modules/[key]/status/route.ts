...
export async function POST(request: Request, context: { params: any }) {
  try {
    ...
    const updated = await updateModuleByKey(key, { is_active: !!body.active || !!body.is_active || !!body.active === false ? Boolean(body.active) : undefined });
    return NextResponse.json({ ok: true, result: updated });
  } catch (err: any) {
    console.error('[POST /api/admin/modules/[key]/status] error', err && err.stack ? err.stack : err);
