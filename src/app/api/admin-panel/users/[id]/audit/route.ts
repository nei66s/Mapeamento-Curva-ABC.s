...
export async function GET(_: Request, { params }: { params: any }) {
  ...
  const res = await pool.query('select id, user_id, entity, entity_id, action, before_data, after_data, ip, user_agent, created_at from audit_logs where entity_id = $1 or user_id::text = $1 order by created_at desc limit 200', [p.id]);
