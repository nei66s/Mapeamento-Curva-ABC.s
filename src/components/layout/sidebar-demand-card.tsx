...
export function SidebarDemandCard() {
  ...
  return (
    <div className="rounded-2xl border px-4 py-3 text-sm shadow-sm">
      <div className="mb-1 flex items-center justify-between text-xs uppercase tracking-[0.3em] text-muted-foreground">
        <span>Demanda</span>
        <button
          type="button"
          onClick={refresh}
          className="text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-primary hover:text-primary-foreground"
        >
          Atualizar
        </button>
      </div>
      <div className="flex items-center gap-2 text-base font-semibold">
        ...
        <span>{severityText[status.severity]}</span>
      </div>
