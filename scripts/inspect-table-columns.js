...
const table = parseArg('-t','--table') || 'stores';
...
(async () => {
  try {
    ...
    if (res.rowCount === 0) {
      console.log(`No columns found for table "${table}". The table may not exist in this database/schema.`);
    } else {
    ...
    console.error('Error inspecting table columns:', err.message || err);
